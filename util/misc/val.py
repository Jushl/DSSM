...
class DetectionValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        for batch_i, batch in enumerate(self.dataloader.dataloader):
            ...
            if self.dataloader.dataset.ttransforms:
            ...
            preds, state, feat, gates = model(batch["img"], state)

            # Loss
            if self.training:
                self.loss += model.loss(batch, state, preds=preds)[0][1]
        ...
        if self.training:
            model.float()
            results = {**stats, **trainer.label_loss_items(self.loss.cpu() / len(self.dataloader.dataloader), prefix="val")}
