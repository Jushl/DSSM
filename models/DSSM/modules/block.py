...
class SVM(nn.Module):
    def __init__(self, d_model=64, num_heads=8, A_init_range=(1, 16)):
        super().__init__()
        self.in_channels = self.state_dim = d_model
        self.num_heads = num_heads
        assert self.state_dim % num_heads == 0
        self.head_dim = self.state_dim // num_heads
        self.in_proj = nn.Conv2d(self.in_channels, self.state_dim, 1)
        self.param_proj = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(self.in_channels, 3 * self.state_dim, 1),
            nn.SiLU()
        )
        A = torch.empty((num_heads, self.head_dim, self.head_dim), dtype=torch.float32).uniform_(*A_init_range)
        A_log = torch.log(A)
        self.A = nn.Parameter(A_log)
        self.A._no_weight_decay = True
        self.out_proj = nn.Conv2d(self.state_dim, self.in_channels, 1)

    def discretize(self, delta):
        delta = delta.view(-1, self.num_heads, self.head_dim, 1).repeat(1, 1, 1, self.head_dim)  # [B, H, D_h, 1, 1]
        A = torch.exp(delta * self.A.unsqueeze(0))  # [B, H, D_h, D_h]
        return A

    def forward(self, x, state=None):
        B_, C_, H_, W_ = x.shape
        x_proj = self.in_proj(x)  # [B, D, H, W]
        params = self.param_proj(x)  # [B, 3*D, 1, 1]
        delta, B, C = torch.split(params, self.state_dim, dim=1)  # [B, D, 1, 1]
        if state is None:
            state = torch.zeros_like(x)

        state = state.view(B_, self.num_heads, self.head_dim, H_, W_)  # [B, H, D_h, H, W]
        x_proj = x_proj.view(B_, self.num_heads, self.head_dim, H_, W_)
        A_discrete = self.discretize(delta)  # [B, H, D_h, D_h]
        state = torch.einsum('bcdhw,bcij->bcdhw', state, A_discrete) + \
                B.view(B_, self.num_heads, self.head_dim, 1, 1) * x_proj
        output = torch.einsum('bcdhw,bcd->bcdhw', state, C.view(B_, self.num_heads, self.head_dim))  # [B, C, H, W]
        state = state.view(B_, self.state_dim, H_, W_)
        output = self.out_proj(output.view(B_, self.state_dim, H_, W_))
        return output, state
...
class DWSConvLSTM2d(nn.Module):
    ...
    def forward(self, x, hc_previous):
        ...
        return h_t1, (h_t1, c_t1), None, None
...
def minimal_discrete(X, A, B, C, block_len: int, initial_states):
...
def mini_chunk_scan_combined(X, dt, A, B, C, chunk_size: int, state):
    Y, final_state = minimal_discrete(X * dt.unsqueeze(-1), A * dt, B, C, chunk_size, state)
    return Y, final_state
...
class StreamingMamba2(nn.Module):
    ...
    def forward(self, u, state):
        ...
        y, state = mini_chunk_scan_combined(
...
class SelectiveMemoryBlock(nn.Module):
    def __init__(self, dim: int = 256,):
        super().__init__()
        self.conv_feat = Conv(dim, dim, 3, 1)
        ...
        self.proj_state = nn.Conv2d(dim, dim, 3, padding=1, groups=dim, bias=False)
        self.proj_feat = nn.Conv2d(dim, dim, 3, padding=1, groups=dim, bias=False)
    ...
    def forward(self, x, state):
        ...
        g_feat = self.gate_feat(fused)
        g_state = self.gate_state(fused)

        new_state = g_state * self.proj_state(state) + g_feat * self.proj_feat(feat)
        out = self.proj_out(new_state)

        return out, new_state, (g_state, g_feat), feat
...
class SST(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)
        self.m1 = SelectiveMemoryBlock(dim=self.c)
        if n > 1:
    ...
    def forward(self, x, state):
        y = list(self.cv1(x).chunk(2, 1))
        out, state, gates, feat = self.m1(y[-1], state)
        y.append(out)
        if self.n > 1:
            y.extend(m(y[-1]) for m in self.m2)
        return self.cv2(torch.cat(y, 1)), state, gates, feat
