...
from models.DSSM.modules.head import Detect
from models.DSSM import unfreeze_streaming_layers


def update_tensor(prev_idx, curr_idx, states, feats):
    if prev_idx is None:
        new_states = None
        new_feats = None
    else:
        new_states, new_feats = [], []
        for p, c, s, f in zip(prev_idx, curr_idx, states, feats):
            if p != c:
                new_states.append(torch.zeros_like(s) if s is not None else None)
                new_feats.append(torch.zeros_like(f) if f is not None else None)
            else:
                new_states.append(s)
                new_feats.append(f)
    ...
    return new_states, new_feats, curr_idx
...
class Detection(object):
    def __init__(
        ...
        iterations = math.ceil(len(dataloader_train.dataloader) / max(self.args.batch_size_train, self.args.nbs)) * self.args.epochs
        ...
        self.scheduler.last_epoch = self.args.epoch - 1

    def train(self):
        nb = len(self.dataloader_train.dataloader)
        ...
        self.epoch = epoch
        tepoch = self.args.epochs - self.args.tepochs
        self.optimizer.zero_grad()
        while True:
            if not epoch < self.args.epochs:
                print("train done...")
                break

            self.dataloader_train.epoch = epoch
            self.dataloader_val.epoch = epoch

            if epoch == tepoch:
                unfreeze_streaming_layers(self.model)
                self.dataloader_train.init_dataloader()
                self.dataloader_train.dataset.start_temporal()

                self.dataloader_val.init_dataloader(trainer=False)
                self.dataloader_val.dataset.start_temporal()
            ...
            self.tloss = None
            prev_idx = None
            state, feat = None, None
            self.dataloader_train.dataset._build_flip_decision()
            for i, batch in enumerate(self.dataloader_train.dataloader):
                ni = i + nb * epoch
                if ni <= nw:
                    xi = [0, nw]
                ...
                with autocast(self.args.amp):
                    ...
                    if self.dataloader_train.dataset.ttransforms:
                        if _idx not in _decisions:
                            state, feat, prev_idx = update_tensor(prev_idx, _idx, state, feat)
                    ...
                    (self.loss, self.loss_items), state, feat = self.model(batch, state, feat)
    ...
    def val(self):
        ...
        self.dataloader_val.epoch = self.args.epochs - self.args.tepochs
        self.dataloader_val.init_dataloader(trainer=False)
        self.dataloader_val.dataset.start_temporal()
    ...
    def load_resume_state(self, inplace=True):
        ...
        self.accumulate = max(round(self.args.nbs / self.args.batch_size_train), 1)  # accumulate loss before optimizing
        weight_decay = self.args.weight_decay * self.args.batch_size_train * self.accumulate / self.args.nbs  # scale weight_decay
        iterations = math.ceil(len(self.dataloader_train.dataloader) / max(self.args.batch_size_train, self.args.nbs)) * self.args.epochs
        self.optimizer = self.build_optimizer(
            model=self.model,
            name=self.args.optimizer,
            lr=self.args.lr0,
    ...
    def get_validator(self):
        ...
        return DetectionValidator(self.dataloader_val, save_dir=self.save_dir, args=copy(self.args))
